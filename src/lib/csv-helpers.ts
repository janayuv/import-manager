...
export const importItemsFromCsv = (
  ...
  try {
    ...
    for (const row of parsedData) {
      ...
      const rawPart = sanitizeString(row.partNumber || '');
      if (!rawPart) {
        skippedCount++;
        continue;
      }
      if (existingPartNumbers.has(rawPart) || seenInImportBatch.has(rawPart)) {
        skippedCount++;
        continue;
      }
      seenInImportBatch.add(rawPart);

      const rawSupplierId = sanitizeString(
        row.supplierId || row.supplierID || row.SUPPLIERID || ''
      );
      const rawSupplierName = sanitizeString(row.supplierName || '');

      let resolvedSupplierId: string | undefined;

      if (rawSupplierId) {
        const supplierById = suppliers.find(
          s => s.value.toLowerCase() === rawSupplierId.toLowerCase()
        );
        if (supplierById) {
          resolvedSupplierId = supplierById.value;
        }
      }

      if (!resolvedSupplierId && rawSupplierName) {
        const supplierByName = suppliers.find(
          s => s.label.toLowerCase() === rawSupplierName.toLowerCase()
        );
        if (supplierByName) {
          resolvedSupplierId = supplierByName.value;
        }
      }

      if (!resolvedSupplierId && (rawSupplierId || rawSupplierName)) {
        validationResult.warnings.push({
          row: rowNumber,
          column: 'supplier',
          message: `Supplier '${rawSupplierId || rawSupplierName}' was not found. Supplier will be left empty for this item.`,
          severity: 'warning',
        });
      }
...
export const generateCsvTemplate = (
  ...
  const templates = {
    items: [
      'partNumber,itemDescription,unit,currency,unitPrice,hsnCode,supplierId,supplierName,isActive,countryOfOrigin,bcd,sws,igst,technicalWriteUp,category,endUse,netWeightKg,purchaseUom,grossWeightPerUomKg,photoPath',
      'ABC-001,Sample Item,PC,USD,10.50,HSN123,Sup-001,Sample Supplier,true,China,5.0,2.5,18.0,Technical description,Electronics,Industrial,0.5,KG,0.6,',
    ],
    shipments: [
