...
export class PerformanceObserver {
  ...
  private observeLargestContentfulPaint() {
    if ('PerformanceObserver' in window) {
      const observer = new globalThis.PerformanceObserver(
...
export class BundleAnalyzer {
  ...
  static getBundleStats() {
    const totalChunkSize = Array.from(this.chunks.values()).reduce(
      (sum, size) => sum + size,
      0
    );
    const totalModuleSize = Array.from(this.modules.values()).reduce(
      (sum, size) => sum + size,
      0
    );
...
export class CodeSplitter {
  ...
  static preloadChunk(chunkName: string, importFn: () => Promise<unknown>) {
    if (
      !this.loadedChunks.has(chunkName) &&
    ...
    const sum = this.reports.reduce(
      (acc, report) => ({
        ...
        firstContentfulPaint:
          (acc.firstContentfulPaint || 0) + report.firstContentfulPaint,
        largestContentfulPaint:
          (acc.largestContentfulPaint || 0) + report.largestContentfulPaint,
        firstInputDelay: (acc.firstInputDelay || 0) + report.firstInputDelay,
        cumulativeLayoutShift:
          (acc.cumulativeLayoutShift || 0) + report.cumulativeLayoutShift,
