...
const splitByDelimiter = (line: string, delimiter: PasteParseOptions['delimiter']): string[] => {
  switch (delimiter) {
    case 'tab':
      return line.split('\t')
    case 'comma':
...
const mapColumns = (cols: string[]): Omit<ParsedPasteLine, 'raw'> => {
  ...
  const isLikelyPart = /[A-Za-z]/.test(trimmed[0] || '') && /[\w.-]/.test(trimmed[0] || '')
  ...
  const qIdx = 3
  const pIdx = 4
  if (isLikelyPart && toNumber(trimmed[qIdx]) !== undefined && toNumber(trimmed[pIdx]) !== undefined) {
    result.partNumber = trimmed[0]
...
export function parseMultiLinePaste(text: string, options: PasteParseOptions = {}): ParsedPasteLine[] {
  const lines = text
    .replace(/\r\n?/g, '\n')
    .split('\n')
    .map((l) => l.trim())
  ...
  const delimiter = options.delimiter && options.delimiter !== 'auto' ? options.delimiter : detectDelimiter(lines[0])
  ...
  lines.forEach((rawLine, index) => {
    ...
    if (mapped.quantity === undefined || mapped.quantity <= 0) errors.push('Invalid quantity')
    if (mapped.unitPrice === undefined || mapped.unitPrice < 0) errors.push('Invalid unit price')
    parsed.push({ raw: rawLine, ...mapped, errors: errors.length ? errors : undefined })
  })

  return parsed
}

export function normalizePartNumber(partNumber: string | undefined): string | undefined {
