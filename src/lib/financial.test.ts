...
import { computePerUnitDuty, computeLandedCostPerUnit, computePotentialDuty, computeDutySavings, computeDutyFromRates, computeSavingsFromActualVsBoe, round } from './financial';
...
describe('financial helpers', () => {
  ...
  it('computes potential duty using standard rates', () => {
    ...
    const result = computePotentialDuty(assessable, { bcdRate: 10, swsRate: 10, igstRate: 18 });
  ...
  it('computes actual duty from rates same as potential helper', () => {
    ...
    const result = computeDutyFromRates(assessable, { bcdRate: 15, swsRate: 10, igstRate: 28 });
    const expected = computePotentialDuty(assessable, { bcdRate: 15, swsRate: 10, igstRate: 28 });
  ...
  it('computes savings only for CEPA/Rodtep and not for Standard', () => {
    ...
    const boeLow = computeDutyFromRates(assessable, { bcdRate: 5, swsRate: 10, igstRate: 18 });
    const boeHigh = computeDutyFromRates(assessable, { bcdRate: 12, swsRate: 10, igstRate: 18 });
    expect(computeSavingsFromActualVsBoe({ method: 'Standard', assessableValue: assessable, actualRates: actual, boe: boeLow })).toBe(0);
    const cepaSaving = computeSavingsFromActualVsBoe({ method: 'CEPA', assessableValue: assessable, actualRates: actual, boe: boeLow });
    expect(cepaSaving).toBeGreaterThan(0);
    const rodtepSaving = computeSavingsFromActualVsBoe({ method: 'Rodtep', assessableValue: assessable, actualRates: actual, boe: boeHigh });
