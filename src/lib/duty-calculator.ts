...
export function calculateDuties({
  ...
  shipment.items.forEach(item => {
    ...
    const itemFreight = round(
      (formValues.freightCost / totalInvoiceValue) * itemForexValue,
      2
    );
    const itemExw = round(
      (formValues.exwCost / totalInvoiceValue) * itemForexValue,
      2
    );
    const itemInsurance = round(
      (itemValueInr + itemExw) * (formValues.insuranceRate / 100),
      1
    );

    const assessableValue = round(
      itemValueInr + itemFreight + itemExw + itemInsurance,
      1
    );
    ...
    switch (correspondingInput.calculationMethod) {
      ...
      case 'CEPA':
        ...
        igstValueRaw =
          (assessableValue + bcdValueRaw + swsValueRaw) * boeIgstRate;
        break;
      ...
      case 'Rodtep': {
        ...
        igstValueRaw =
          (assessableValue + actualBcdValueForIgst + swsValueRaw) * boeIgstRate;
        break;
      }
    }
  ...
  const bcdTotal = Math.round(
    calculatedItems.reduce((sum, item) => sum + item.bcdValue, 0)
  );
  const swsTotal = Math.round(
    calculatedItems.reduce((sum, item) => sum + item.swsValue, 0)
  );
  const igstTotal = Math.round(
    calculatedItems.reduce((sum, item) => sum + item.igstValue, 0)
  );
  const interest = formValues.interest || 0;

  // Final total rounded to the nearest whole number
  const customsDutyTotal = Math.round(
