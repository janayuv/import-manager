...
import { buildShipmentImportTemplateCsv } from '@/lib/shipment-import';
...
export function generateShipmentTemplate(): string {
  const pasteFormats = `Supplier,Invoice Number,Invoice Date,Goods Category,Invoice Value,Currency,Incoterm,Shipment Mode,Shipment Type,BL/AWB Number,BL/AWB Date,Vessel Name,Container Number,Gross Weight (KG),ETD,ETA,Status,Date of Delivery
...
CNF Co Ltd,01-07-2025,Non Wove,39703392,KRW,EXW,FCL,40 FTx2,MVMX209,08-07-2025,HMM OCE,CAIU77085,6576,08-07-2025,30-07-2025,docs-rcvd`;
  ...
  const csvImportBlock = buildShipmentImportTemplateCsv()
    .replace(/^\uFEFF/, '')
    .trimEnd();

  return `${pasteFormats}

--- CSV file import (Shipments screen uses the same columns; comma, tab, or semicolon) ---
${csvImportBlock}`;
}
