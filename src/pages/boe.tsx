...
const BoePage = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      const filePath = await save({ defaultPath: 'boe_details.csv', filters: [{ name: 'CSV', extensions: ['csv'] }] });
      if (filePath) {
  ...
  const handleDownloadTemplate = async () => {
     const templateData = [{
        beNumber: "1234567", beDate: "2024-12-31", location: "INMAA1",
        totalAssessmentValue: "10000.50", dutyAmount: "2500.75", paymentDate: "2025-01-15",
        dutyPaid: "2500.75", challanNumber: "CH123", refId: "REF456", transactionId: "TRN789"
    }];
    const csv = Papa.unparse(templateData);
     try {
      const filePath = await save({ defaultPath: 'boe_template.csv', filters: [{ name: 'CSV', extensions: ['csv'] }] });
      if (filePath) {
        await writeTextFile(filePath, csv);
    ...
    <div className="container mx-auto py-10">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-3xl font-bold">Bill of Entry Details</h1>
         <div className="flex items-center gap-2">
            <Button onClick={handleDownloadTemplate} variant="outline" style={{ backgroundColor: '#a855f7', color: 'white' }}>
                <Download className="mr-2 h-4 w-4" /> Template
            </Button>
            <Button onClick={handleImport} variant="outline" style={{ backgroundColor: '#22c55e', color: 'white' }}>
                <Upload className="mr-2 h-4 w-4" /> Import
            </Button>
            <Button onClick={handleExport} variant="outline" style={{ backgroundColor: '#3b82f6', color: 'white' }}>
                <Download className="mr-2 h-4 w-4" /> Export
            </Button>
            <Button onClick={handleOpenFormForAdd} style={{ backgroundColor: '#e7739e', color: 'white' }}>
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <AlertDialogContent>
            ...
            <AlertDialogFooter>
                <AlertDialogCancel onClick={()  => setBoeToDelete(null)} className="custom-alert-action-ok">Cancel</AlertDialogCancel>
                <AlertDialogAction onClick={handleDeleteConfirm} className="custom-alert-action-orange">Continue</AlertDialogAction>
            </AlertDialogFooter>
        </AlertDialogContent>
    </AlertDialog>
    </div>
