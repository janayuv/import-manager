...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    fetchAll();
    // Run once on mount: `notifications` from context is a new object each render,
    // so listing `notifications.system` here re-ran the effect continuously and kept
    // the dashboard stuck in its loading skeleton (bad for web/Vite smoke tests).
    // eslint-disable-next-line react-hooks/exhaustive-deps -- stable mount-only fetch; see comment above
  }, []);
