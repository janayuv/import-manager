...
const ShipmentSelector: React.FC<ShipmentSelectorProps> = ({
    ...
    try {
      ...
      const shipmentsWithExpenses = await Promise.all(
        fetchedShipments.map(async shipment => {
          try {
            const expenses: Expense[] = await invoke(
            ...
            console.error(
              `Failed to fetch expenses for shipment ${shipment.id}:`,
              error
            );
            return {
              ...shipment,
  ...
  const getSelectedShipmentWithExpenses =
    (): ShipmentWithExpenseCount | null => {
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Freezing will hide this shipment from the expense tracking
                  list but won't delete the shipment or expenses.
                  <br />
                  <br />
        ...
        <div className="bg-muted/50 mt-3 rounded-md border p-3">
          <div className="mb-2 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {selectedShipmentWithExpenses &&
                getStatusBadge(selectedShipmentWithExpenses)}
              {selectedShipment && (
                <Button
                  size="sm"
                  variant="outline"
          ...
          <div className="text-muted-foreground space-y-1 text-xs">
            ...
            <p>
              <strong>Date:</strong>{' '}
              {safeDateLabel(selectedShipment.invoiceDate)}
            </p>
            {selectedShipmentWithExpenses &&
              selectedShipmentWithExpenses.expenseCount > 0 && (
                <p>
                  <strong>Expenses:</strong>{' '}
                  {selectedShipmentWithExpenses.expenseCount} (₹
                  {formatNumber(
          ...
          {selectedShipmentWithExpenses &&
            getCompletionSuggestion(selectedShipmentWithExpenses)}
        </div>
      )}
    </div>
  );
