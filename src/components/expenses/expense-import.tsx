...
export default function ExpenseImport({
    ...
    (data: ImportExpenseRow[]): string[] => {
      ...
      data.forEach((row, index) => {
        ...
        if (row.invoiceDate && isNaN(Date.parse(row.invoiceDate))) {
          errors.push(`Row ${rowNumber}: Invalid Invoice Date format (use YYYY-MM-DD)`)
        }
        ...
        if (isNaN(row.cgstAmount) || row.cgstAmount < 0) {
          errors.push(`Row ${rowNumber}: CGST Amount must be a non-negative number`)
        }
        if (isNaN(row.sgstAmount) || row.sgstAmount < 0) {
          errors.push(`Row ${rowNumber}: SGST Amount must be a non-negative number`)
        }
        if (isNaN(row.igstAmount) || row.igstAmount < 0) {
          errors.push(`Row ${rowNumber}: IGST Amount must be a non-negative number`)
        }
        if (isNaN(row.tdsAmount) || row.tdsAmount < 0) {
          errors.push(`Row ${rowNumber}: TDS Amount must be a non-negative number`)
        }
        if (isNaN(row.totalAmount) || row.totalAmount <= 0) {
          errors.push(`Row ${rowNumber}: Total Amount must be a positive number`)
        }
        ...
        if (!serviceProvider) {
          errors.push(`Row ${rowNumber}: Service Provider "${row.serviceProviderName}" not found`)
        }
      })
  ...
  const handleFileUpload = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        setValidationErrors(['Failed to parse file. Please ensure it matches the template format.'])
      } finally {
  ...
  const parseCSV = async (file: File): Promise<ImportExpenseRow[]> => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      data.push({
        ...
        serviceProviderName: row['Service Provider'] || row['serviceProviderName'] || '',
  ...
  const parseExcel = async (file: File): Promise<ImportExpenseRow[]> => {
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      data.push({
        expenseTypeName: rowData['Expense Type'] || rowData['expenseTypeName'] || '',
        serviceProviderName: rowData['Service Provider'] || rowData['serviceProviderName'] || '',
        ...
        cgstAmount: parseFloat(rowData['CGST Amount'] || rowData['cgstAmount'] || '0'),
        sgstAmount: parseFloat(rowData['SGST Amount'] || rowData['sgstAmount'] || '0'),
        igstAmount: parseFloat(rowData['IGST Amount'] || rowData['igstAmount'] || '0'),
        tdsAmount: parseFloat(rowData['TDS Amount'] || rowData['tdsAmount'] || '0'),
        totalAmount: parseFloat(rowData['Total Amount'] || rowData['totalAmount'] || '0'),
            ...
            {selectedShipmentData && (
              <div className="mt-2">
                <Badge variant="outline">
                  Selected: {formatText(selectedShipmentData.invoiceNumber, settings.textFormat)}
                </Badge>
              </div>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-4">
              <input
                ...
                className="flex-1"
              />
              <div className="text-muted-foreground text-sm">Supports CSV, Excel (.xlsx, .xls)</div>
            </div>
          </div>
              ...
              <Table>
                ...
                <TableBody>
                  {importData.slice(0, 10).map((row, index) => (
                    <TableRow key={index}>
                      <TableCell className="font-medium">{row.expenseTypeName}</TableCell>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <ul className="text-muted-foreground space-y-1 text-sm">
                ...
                <li>• Expense Type and Service Provider names must match existing records</li>
              </ul>
            </div>
            <div className="space-y-2">
