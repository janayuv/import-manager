...
export const BoeForm: React.FC<BoeFormProps> = ({ isOpen, onOpenChange, onSubmit, boeToEdit, existingBoes }) => {
    ...
    const validate = () => {
        ...
        const isDuplicate = existingBoes.some(
            boe => boe.beNumber === formData.beNumber && 
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4 pt-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <Input id="beNumber" name="beNumber" value={formData.beNumber} onChange={handleChange} />
                    {errors.beNumber && <p className="text-red-500 text-xs mt-1">{errors.beNumber}</p>}
                </div>
                 <div>
                    <Label htmlFor="beDate">BE Date</Label>
                    <Input id="beDate" name="beDate" type="date" value={formData.beDate} onChange={handleChange} />
                    {errors.beDate && <p className="text-red-500 text-xs mt-1">{errors.beDate}</p>}
                </div>
                <div>
                ...
                <div>
                    ...
                    <Input id="totalAssessmentValue" name="totalAssessmentValue" type="number" value={formData.totalAssessmentValue} onChange={handleNumberChange} />
                </div>
                <div>
                    <Label htmlFor="dutyAmount">Duty Amount</Label>
                    <Input id="dutyAmount" name="dutyAmount" type="number" value={formData.dutyAmount} onChange={handleNumberChange} />
                </div>
                <div>
                 ...
                 <div>
                    ...
                    <Input id="dutyPaid" name="dutyPaid" type="number" value={formData.dutyPaid} onChange={handleNumberChange} />
                </div>
                <div>
                    <Label htmlFor="challanNumber">Challan No.</Label>
                    <Input id="challanNumber" name="challanNumber" value={formData.challanNumber} onChange={handleChange} />
                </div>
                 <div>
            ...
            <DialogFooter>
              <DialogClose asChild>
                <Button type="button" style={{ backgroundColor: '#d73000', color: 'white' }}>Cancel</Button>
              </DialogClose>
              <Button type="submit" style={{ backgroundColor: '#13fd00', color: 'white' }}>{boeToEdit ? 'Update' : 'Save'}</Button>
            </DialogFooter>
          </form>
      </DialogContent>
    </Dialog>
