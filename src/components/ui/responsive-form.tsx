...
export const ResponsiveFormField: React.FC<ResponsiveFormFieldProps> = ({
  ...
  const variantClasses = {
    ...
    compact: 'space-y-1',
    horizontal:
      'flex flex-col sm:flex-row sm:items-center sm:justify-between sm:space-y-0 sm:space-x-4',
  };
  ...
  return (
    <div className={cn(variantClasses[variant], className)}>
      ...
      {description && (
        <p className={cn(getTextClass('xs'), 'text-muted-foreground')}>
          {description}
        </p>
      )}
      <div className={getSpacingClass()}>{children}</div>
      {error && (
        <p className={cn(getTextClass('xs'), 'text-destructive')}>{error}</p>
      )}
    </div>
  );
};
...
export const ResponsiveFormSection: React.FC<ResponsiveFormSectionProps> = ({
  ...
  const { getTextClass, getSpacingClass, getCardClass } =
    ...
    <div className={cn(variantClasses[variant], className)}>
      {(title || description) && (
        <div className={getSpacingClass()}>
          {title && (
            <h3 className={cn(getTextClass('lg'), 'font-semibold')}>{title}</h3>
          )}
          {description && (
            <p className={cn(getTextClass('sm'), 'text-muted-foreground')}>
              {description}
            </p>
          )}
        </div>
...
export const ResponsiveFormActions: React.FC<ResponsiveFormActionsProps> = ({
  ...
  const variantClasses = {
    ...
    sticky:
      'sticky bottom-0 bg-background border-t p-4 flex items-center justify-end space-x-2',
  };
