...
import { parseMultiLinePaste, type ParsedPasteLine } from '@/lib/multiline-paste'
...
function getLatestDraftForShipment(shipmentId: string | undefined): InvoiceWizardDraft | undefined {
...
export function InvoiceWizard({ shipments, items, suppliers, invoices, onSubmit }: InvoiceWizardProps) {
  ...
  const supplierOptions: ComboboxOption[] = suppliers.map((s) => ({ value: s.id, label: s.supplierName }))
  ...
  const availableShipments = React.useMemo(() => {
    ...
    const shipmentIdsWithInvoices = new Set(invoices.map((inv) => inv.shipmentId))

    const filtered = shipments.filter((s) => {
      // Filter out shipments with status "Finalized", "Closed", or similar
      const status = s.status?.toLowerCase() || ''
      const isFinalized = status.includes('finalized') || status.includes('closed') || status.includes('completed')
  ...
  const selectedShipment = React.useMemo(() => {
    const found = header.shipmentId ? availableShipments.find((s) => s.id === header.shipmentId) : undefined
  ...
  React.useEffect(() => {
    ...
    if (header.shipmentId) {
      ...
      if (sh) {
        ...
        setShipmentFrozen(!!sh.isFrozen || (sh.status || '').toLowerCase().includes('final'))
      }
    }
  }, [header.shipmentId, availableShipments])
  ...
  const itemIdOptions: ComboboxOption[] = availableItems.map((i) => ({ value: i.id, label: i.partNumber }))
  ...
  const stepsCount = 3
  const currentProgress = step === 1 ? 100 / stepsCount : step === 2 ? (100 / stepsCount) * 2 : (100 / stepsCount) * 3
  ...
  const validateHeader = () => {
    ...
    if (header.shipmentId) {
      const selectedShipment = availableShipments.find((s) => s.id === header.shipmentId)
      if (!selectedShipment) {
        return false
      }

      // Check if shipment already has invoices
      const existingInvoices = invoices.filter((inv) => inv.shipmentId === header.shipmentId)
  ...
  const handlePasteParse = () => {
    ...
    const parsed = parseMultiLinePaste(pasteText, { delimiter: 'auto', skipHeader: false })
    ...
    const parsedWithWarnings = parsed.map((line) => {
      ...
      if (matchedItem && line.unitPrice !== undefined && line.unitPrice !== matchedItem.unitPrice) {
        return {
          ...line,
          priceWarning: {
  ...
  const acceptParsedPreview = () => {
    if (itemsLoading) {
      toast.warning('Items are still loading. Please wait before adding lines.')
      return
    }
    if (items.length === 0) {
      toast.warning('Item Master not loaded yet. Please wait.')
  ...
  const updateLine = (id: string, field: keyof InvoiceWizardDraftLineItem, value: string | number) => {
  ...
  const handleSubmit = async () => {
    if (!validateHeader()) {
      ...
      if (header.shipmentId) {
        const selectedShipment = availableShipments.find((s) => s.id === header.shipmentId)
        if (!selectedShipment) {
          toast.error('Selected shipment is not available or has been finalized. Please select an active shipment.')
          setStep(1)
          return
        }
        ...
        const existingInvoices = invoices.filter((inv) => inv.shipmentId === header.shipmentId)
        if (existingInvoices.length > 0) {
          toast.error('This shipment already has an invoice. Please choose another.')
          setStep(1)
          return
        }
      }
    ...
    const selectedShipment = header.shipmentId ? availableShipments.find((s) => s.id === header.shipmentId) : undefined
    const payload: Omit<Invoice, 'id'> = {
      invoiceNumber: selectedShipment?.invoiceNumber || header.invoiceNumber,
      shipmentId: selectedShipment?.id || '',
      invoiceDate: selectedShipment?.invoiceDate || formatDateForInput(header.invoiceDate),
      status: 'Draft',
      calculatedTotal: Math.round(totalCalculated * 100) / 100,
      shipmentTotal: selectedShipment?.invoiceValue || Math.round(totalCalculated * 100) / 100,
      lineItems: lines.map((l) => ({ id: l.id, itemId: l.itemId, quantity: l.quantity, unitPrice: l.unitPrice })),
    }
  ...
  const draftList = React.useMemo(() => readDrafts().sort((a, b) => b.updatedAt - a.updatedAt), [])
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6">
      <div className="space-y-2">
        ...
        {loadedFromDraft && <div className="text-muted-foreground text-xs">Draft loaded</div>}
        <div className="text-muted-foreground text-xs">
          Step {step} of {stepsCount}: {step === 1 ? 'Invoice Header' : step === 2 ? 'Invoice Lines' : 'Review & Save'}
        </div>
      </div>
      ...
      {step === 1 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Invoice number/date, supplier, currency, and shipment.</CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
            <div>
              ...
              <Combobox
                ...
                onChange={(v) => setHeader((h) => ({ ...h, shipmentId: v || undefined }))}
                placeholder={availableShipments.length > 0 ? 'Select shipment (optional)' : 'No available shipments'}
                disabled={availableShipments.length === 0}
                emptyText={availableShipments.length === 0 ? 'No available shipments' : 'No shipments found'}
              />
              <p className="text-muted-foreground mt-1 text-xs">Select from shipments without existing invoices.</p>
            </div>
            <div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setHeader((h) => ({ ...h, invoiceDate: formatDateForDisplay(e.target.value) }))}
              />
            </div>
            <div>
              ...
              <div className="md:col-span-2 lg:col-span-4">
                ...
                <div className="text-muted-foreground bg-muted rounded p-2 text-sm font-medium">
                  {new Intl.NumberFormat('en-IN', {
                    style: 'currency',
                    currency: normalizeCurrencyCode(selectedShipment.invoiceCurrency) || 'INR',
                  }).format(selectedShipment.invoiceValue || 0)}
                </div>
                <p className="text-muted-foreground mt-1 text-xs">Total invoice value linked to this shipment</p>
              </div>
            )}
            ...
            {process.env.NODE_ENV === 'development' && (
              <div className="md:col-span-2 lg:col-span-4">
                ...
                <div className="text-muted-foreground bg-muted rounded p-2 text-xs">
                  ...
                  <div>Selected Shipment Found: {selectedShipment ? 'Yes' : 'No'}</div>
                </div>
              </div>
            )}
            <div className="md:col-span-2 lg:col-span-4">
      ...
      {step === 2 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Paste multiple lines or add manually. Only items of the selected supplier are allowed.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {itemsLoading && (
                    <div className="text-muted-foreground flex items-center gap-1 text-xs">
                      <Loader2 className="h-3.5 w-3.5 animate-spin" /> Loading Item Master...
                    </div>
                  )}
                </div>
                <Textarea
                    ...
                    <TableBody>
                      {parsedPreview.map((p, idx) => (
                        <TableRow
                          key={idx}
                          className={
                            p.errors?.length ? 'bg-destructive/10' : p.matched === false ? 'bg-warning/10' : ''
                          }
                        >
                          <TableCell>
                            {p.matched === false ? (
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <div className="flex items-center gap-1">
                                    <span className="text-warning-foreground">{p.partNumber || '-'}</span>
                                    <span className="text-warning-foreground text-xs">⚠️</span>
                                  </div>
                                </TooltipTrigger>
                                <TooltipContent>
                                  <div className="text-xs">Item not found in Item Master</div>
                                </TooltipContent>
                              </Tooltip>
                            ) : (
                              p.partNumber || '-'
                          ...
                          <TableCell>
                            {p.priceWarning ? (
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <div className="flex items-center gap-1">
                                    <span className="text-warning-foreground">{p.unitPrice}</span>
                                    <span className="text-warning-foreground text-xs">⚠️</span>
                                  </div>
                                </TooltipTrigger>
                                <TooltipContent>
                                  <div className="text-xs">
                                    <div>Item Master: {p.priceWarning.itemMasterPrice}</div>
                                    <div>Pasted: {p.priceWarning.pastedPrice}</div>
                                    <div
                                      className={p.priceWarning.difference > 0 ? 'text-destructive' : 'text-green-600'}
                                    >
                                      Difference: {p.priceWarning.difference > 0 ? '+' : ''}
                    ...
                    return (
                      <TableRow key={line.id}>
                        ...
                        <TableCell>
                          {(() => {
                            const item = items.find((i) => i.id === line.itemId)
                            const hasPriceDifference = item && line.unitPrice !== item.unitPrice
                            ...
                            return (
                              <div className="flex items-center gap-1">
                                <Input
                                  ...
                                  className={hasPriceDifference ? 'border-warning' : ''}
                                />
                                {hasPriceDifference && (
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <span className="text-warning-foreground cursor-help text-xs">⚠️</span>
                                    </TooltipTrigger>
                                    <TooltipContent>
                                      <div className="text-xs">
                                        <div>Item Master: {item?.unitPrice}</div>
                                        <div>Current: {line.unitPrice}</div>
                                        <div
                                          className={
                                            line.unitPrice - (item?.unitPrice || 0) > 0
                                              ? 'text-destructive'
                                              : 'text-green-600'
                                          }
                                        >
                                          Difference: {line.unitPrice - (item?.unitPrice || 0) > 0 ? '+' : ''}
                        ...
                        <TableCell>
                          <Input
                            value={((line.quantity || 0) * (line.unitPrice || 0)).toFixed(2)}
                            readOnly
                          />
                        </TableCell>
                        <TableCell>
            ...
            <div className="flex justify-end text-sm">
              <div className="space-x-6">
                ...
                <span className="text-muted-foreground ml-6">Calculated Total:</span>
            ...
            <div className="mt-4 flex items-center justify-between">
              <div className="text-muted-foreground text-xs">
                {invoiceFinalized ? 'Invoice finalized' : 'Invoice not finalized'} •{' '}
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={
                    ...
                    Math.round(totalCalculated * 100) !==
                      Math.round((availableShipments.find((s) => s.id === header.shipmentId)?.invoiceValue || 0) * 100)
                  }
                  onClick={async () => {
                    ...
                    if (Math.round(totalCalculated * 100) !== Math.round(shipmentTotal * 100)) {
                      return toast.error('Cannot finalize. Calculated total must match shipment value.')
                    }
                    setFinalizing(true)
                    try {
                ...
                >
                  {finalizing && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Finalize Invoice
                </Button>
                <Button
                  variant="secondary"
                  ...
                  onClick={async () => {
                    ...
                    setFinalizing(true)
                    try {
                      await invoke('freeze_shipment', { shipmentId: sh.id, frozen: true })
                ...
                >
                  {finalizing && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Finalize Shipment
                </Button>
              </div>
            </div>
          </CardContent>
      ...
      {step === 3 && (
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
              <div>
                ...
                <div className="text-sm font-medium">
                  {suppliers.find((s) => s.id === header.supplierId)?.supplierName || '-'}
                </div>
              </div>
              <div>
                <Label>Invoice Number</Label>
                <div className="text-sm font-medium">{header.invoiceNumber}</div>
              </div>
              <div>
                <Label>Invoice Date</Label>
            ...
            <div className="flex items-center justify-between">
              <div className="text-muted-foreground text-sm">Currency: {currency}</div>
              <div className="text-right">
                <div className="text-muted-foreground text-sm">Calculated Total</div>
        ...
        <div className="flex gap-2">
          ...
          <Button
            onClick={() => {
              ...
              if (step === 2) {
                if (lines.length === 0) {
                  toast.error('Add at least one invoice line before continuing')
                  return
                }
              }
              const maxStep = 3
            ...
            disabled={(step === 1 && !validateHeader()) || (step === 2 && lines.length === 0)}
          >
            {step < 3 ? 'Next' : 'Finish'}
          </Button>
        </div>
