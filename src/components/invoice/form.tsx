...
import { X, Plus, Upload, Download, Loader2 } from 'lucide-react';
...
export const InvoiceForm: React.FC<InvoiceFormProps> = ({ isOpen, onOpenChange, onSubmit, shipments, items, invoiceToEdit }) => {
  ...
  React.useEffect(() => { setCalculatedTotal(lineItems.reduce((sum, item) => sum + (item.quantity * item.unitPrice), 0)); }, [lineItems]);
  ...
  const handleAddItem = () => setLineItems([...lineItems, { id: `item-${Date.now()}`, itemId: '', quantity: 1, unitPrice: 0 }]);
  ...
  const handleTemplateDownload = async () => {
    const templateData = [{ partNumber: 'EXAMPLE-PN-1', quantity: 10, unitPrice: 12.50 }];
    const csv = Papa.unparse(templateData);
    try {
      const filePath = await save({ defaultPath: 'item_import_template.csv', filters: [{ name: 'CSV', extensions: ['csv'] }] });
      if (filePath) {
        ...
        Papa.parse(content, {
          ...
          complete: (results: Papa.ParseResult<InvoiceLineItemCsvRow>) => {
            ...
            results.data.forEach((row: InvoiceLineItemCsvRow) => {
              ...
              if (!item) { notFoundItems.push(row.partNumber); return; }
              const isDuplicate = lineItems.some(li => li.itemId === item.id) || itemsToAdd.some(li => li.itemId === item.id);
              if (isDuplicate) { duplicateItems.push(row.partNumber); return; }
              itemsToAdd.push({ id: `imported-${Date.now()}-${Math.random()}`, itemId: item.id, quantity: parseFloat(row.quantity) || 0, unitPrice: parseFloat(row.unitPrice) || item.unitPrice });
            });
            
            setLineItems(prev => [...prev, ...itemsToAdd]);
            if (itemsToAdd.length > 0) toast.success(`${itemsToAdd.length} items imported successfully!`);
            if (notFoundItems.length > 0) toast.warning(`Skipped ${notFoundItems.length} items not found: ${notFoundItems.join(', ')}`);
            if (duplicateItems.length > 0) toast.info(`Skipped ${duplicateItems.length} duplicate items: ${duplicateItems.join(', ')}`);
            if (itemsToAdd.length === 0 && notFoundItems.length === 0 && duplicateItems.length === 0) toast.info("No new items were imported.");
          },
          error: (err: Error) => { toast.error("Failed to parse CSV file."); console.error("CSV parsing error:", err); }
        });
      }
    } catch (error) { console.error("Failed to import items:", error); toast.error("Failed to import items."); }
  };
  ...
  const handleSave = (status: 'Draft' | 'Finalized') => {
      ...
      setTimeout(() => {
        if (!selectedShipment) { setIsSubmitting(false); return toast.error("Please select a shipment first."); }
        const isMatched = selectedShipment.invoiceValue === calculatedTotal;
        if (status === 'Finalized' && !isMatched) { setIsSubmitting(false); return toast.error("Cannot finalize. The calculated total must match the shipment value."); }
      ...
      <DialogContent className="sm:max-w-5xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-b pb-4">
            <div>
                ...
                <Combobox options={shipmentOptions} value={selectedShipment?.id || ''} onChange={handleShipmentSelect} placeholder="Select a shipment..." disabled={!!invoiceToEdit}/>
            </div>
             <div><Label>Invoice Number</Label><Input value={selectedShipment?.invoiceNumber || ''} readOnly /></div>
             <div><Label>Invoice Date</Label><Input value={selectedShipment?.invoiceDate || ''} readOnly /></div>
        </div>
        ...
        {selectedShipment && (
            ...
            <div className="space-y-4">
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                        <Button variant="outline" size="sm" onClick={handleTemplateDownload}><Download className="mr-2 h-4 w-4"/> Template</Button>
                        <Button variant="outline" size="sm" onClick={handleItemImport}><Upload className="mr-2 h-4 w-4"/> Import Items</Button>
                    </div>
                </div>
                 <div className="max-h-64 overflow-y-auto pr-2 border rounded-md">
                     <Table>
                        <TableHeader className="sticky top-0 bg-pink-800 z-10">
                            <TableRow>
                        ...
                        <TableBody>
                            {lineItems.map((lineItem) => {
                                ...
                                <TableRow key={lineItem.id}>
                                    ...
                                    <TableCell>
                                        <Input type="number" value={lineItem.quantity} onChange={e => handleLineItemChange(lineItem.id, 'quantity', parseFloat(e.target.value) || 0)}/>
                                    </TableCell>
                                    <TableCell>
                                        <Input type="number" value={lineItem.unitPrice} onChange={e => handleLineItemChange(lineItem.id, 'unitPrice', parseFloat(e.target.value) || 0)}/>
                                    </TableCell>
                                    <TableCell>
                                        <Input value={formatCurrency(lineItem.quantity * lineItem.unitPrice)} readOnly className="bg-gray-100"/>
                                    </TableCell>
                                    <TableCell>
                                        <AlertDialog>
                                            ...
                                            <AlertDialogContent>
                                                <AlertDialogHeader><AlertDialogTitle>Are you sure?</AlertDialogTitle><AlertDialogDescription>This will permanently remove the item from this invoice. This action cannot be undone.</AlertDialogDescription></AlertDialogHeader>
                                                <AlertDialogFooter>
                                                    <AlertDialogCancel className="custom-alert-action-cancel">Cancel</AlertDialogCancel>
                                                    <AlertDialogAction className="custom-alert-action-ok" onClick={() => handleRemoveItem(lineItem.id)}>Continue</AlertDialogAction>
                                                </AlertDialogFooter>
                                            </AlertDialogContent>
                                        </AlertDialog>
                                    </TableCell>
            ...
            <div className="flex justify-end items-center gap-6 pt-4 border-t">
                ...
                <div className="text-right"><p className="text-sm text-gray-500">Calculated Total</p><p className="font-bold text-lg">{formatCurrency(calculatedTotal)}</p></div>
                <div className={`text-xl font-bold flex items-center gap-2 ${isMatch ? 'text-green-600' : 'text-red-600'}`}>
