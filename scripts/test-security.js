...
try {
  const eslintOutput = execSync('npx eslint src/db/test.ts', {
    encoding: 'utf8',
  });
...
try {
  const securityOutput = execSync('node scripts/security-check.js', {
    encoding: 'utf8',
  });
  if (
    ...
    console.log(
      '✅ Custom security scanner correctly detected hardcoded secrets'
    );
  } else if (
    securityOutput.includes('❌ SECURITY:') &&
    ...
    console.log(
      '✅ Custom security scanner correctly detected unencrypted SQLite usage'
    );
  } else {
...
try {
  ...
  if (
    ...
    (preCommitOutput.includes('❌ SECURITY:') ||
      preCommitOutput.includes('Security checks passed'))
  ) {
