...
const SECRET_PATTERNS = [
  pemHeaderPattern('PRIVATE'),
  pemHeaderPattern('RSA PRIVATE'),
  pemHeaderPattern('DSA PRIVATE'),
  pemHeaderPattern('EC PRIVATE'),
  pemHeaderPattern('OPENSSH PRIVATE'),
    ...
    lines.forEach((line, index) => {
      ...
      SECRET_PATTERNS.forEach(pattern => {
        if (pattern.test(line)) {
          console.log(
            `❌ SECURITY: Hardcoded secret detected in ${filePath}:${lineNum}`
          );
      ...
      SQLITE_PATTERNS.forEach(pattern => {
        if (pattern.test(line) && !line.includes('PRAGMA key')) {
          console.log(
            `❌ SECURITY: Unencrypted SQLite usage detected in ${filePath}:${lineNum}`
          );
          console.log(`   ${line.trim()}`);
