...
pub fn add_boe_calculation(payload: SavedBoe, state: State<DbState>) -> Result<String, String> {
    ...
    conn.execute(
        "INSERT INTO boe_calculations (id, shipment_id, boe_id, supplier_name, invoice_number, form_values_json, item_inputs_json, calculation_result_json)
...
pub fn update_boe_calculation(payload: SavedBoe, state: State<DbState>) -> Result<(), String> {
    ...
    conn.execute(
        "UPDATE boe_calculations 
         SET shipment_id = ?2, boe_id = ?3, supplier_name = ?4, invoice_number = ?5, form_values_json = ?6, item_inputs_json = ?7, calculation_result_json = ?8
         WHERE id = ?1",
        params![
            payload.id,
            payload.shipment_id,
