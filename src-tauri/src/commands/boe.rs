...
pub fn get_boes(state: State<DbState>) -> Result<Vec<BoeDetails>, String> {
    ...
    let mut stmt = conn
        .prepare("SELECT * FROM boe_details")
...
pub fn add_boe_calculation(payload: SavedBoe, state: State<DbState>) -> Result<String, String> {
    ...
    let form_values_json =
        serde_json::to_string(&payload.form_values).map_err(|e| e.to_string())?;
    let item_inputs_json =
        serde_json::to_string(&payload.item_inputs).map_err(|e| e.to_string())?;
    let calculation_result_json =
        serde_json::to_string(&payload.calculation_result).map_err(|e| e.to_string())?;
    ...
    for it in &saved.calculation_result.calculated_items {
        ...
        )) = shipment.get(&it.part_no).cloned()
        {
            ...
            let savings = if method == "Standard" {
                ...
                (actual_total - boe_total).max(0.0)
            };
